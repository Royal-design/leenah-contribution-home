...
import { CheckCircle2, ChevronLeft, Headset, Send } from "lucide-react"
...
export default function AdminSupportThreadPage() {
  ...
  return (
    <div className="flex flex-col gap-8">
      <Button variant="ghost" size="sm" className="w-fit" render={<Link href="/admin/support" />}>
        <ChevronLeft />
        All messages
      </Button>
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            From {thread.userName || "Unknown user"}
            {thread.userEmail ? ` · ${thread.userEmail}` : ""} ·{" "}
            {categoryLabels[thread.category] ?? thread.category} · Opened{" "}
      ...
      <Card>
        <CardContent className="flex flex-col gap-4 py-5">
          {thread.messages.map((message) => {
            const fromTeam = message.senderRole === "admin"
            return (
              <div
                key={message.id}
              ...
              >
                <span
                  className={cn(
                    ...
                    fromTeam ? "bg-primary/15 text-primary" : "bg-muted text-muted-foreground"
                  )}
                >
                  {fromTeam ? (
      ...
      <Card>
        <CardHeader>
          <CardTitle>{resolved ? "Conversation closed" : "Reply to user"}</CardTitle>
          <CardDescription>
            {resolved
              ? "This thread is resolved. Change its status to reopen it and reply."
              : "Send a reply to the user on this thread."}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-4">
            <Controller
              ...
              render={({ field }) => (
                <Textarea
                  rows={4}
                  placeholder={resolved ? "Resolve a thread to reply…" : "Type your reply…"}
            ...
            <div className="flex justify-end">
              <Button
                type="submit"
                size="lg"
                disabled={resolved || replyThread.isPending || form.formState.isSubmitting}
              >
                <Send />
      ...
      <ConfirmDialog
        ...
        description="Mark this thread as resolved. The sender can open a new thread if they need more help."
